...
for snr_train in range(0,20,5):
    for part_id in [0,1,2,3]:
        for noise_type_train in train_test_type.keys():    
            for seed in [0]:
                exp_name = f"wind_sub_test_{noise_type_train}({snr_train}dB)"
                ...
                base_cfg['train_augmentation'] = {
                        ...
                        "id": f"AudioSet{noise_type_train}-{part_id}-({snr_train})",
                        "pipeline": [
                            {
                                "AudioSet":{
                                ...
                                "p": 1.0,
                                "generator_seed": 0,
                                "noise_type": f"AudioSet{noise_type_train}_{part_id}",
                                }
                            }
                        ],
                }
                ...
                base_cfg['test_augmentation']['id'] = f'AudioSet{snr}'
                base_cfg['hydra']['sweeper']['params']['+seed'] = seed
                base_cfg['experiment_id'] = 'wind_sub'
